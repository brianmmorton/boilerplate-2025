...
```bash
grant all privileges on database boilerplate to brianmorton;
```

Create Shadow Database to run npx prisma migrate dev

```bash
create database boilerplate_shadow;
```

```bash
grant all privileges on database boilerplate_shadow to brianmorton;
```
...
To view the list of available APIs and their specifications, run the server and go to `http://localhost:3000/1/docs` in your browser. This documentation page is automatically generated using the [swagger](https://swagger.io/) definitions written as comments in the route files.
...
**Auth routes**:\
`POST /1/auth/register` - register\
`POST /1/auth/login` - login\
`POST /1/auth/refresh-tokens` - refresh auth tokens\
`POST /1/auth/forgot-password` - send reset password email\
`POST /1/auth/reset-password` - reset password\
`POST /1/auth/send-verification-email` - send verification email\
`POST /1/auth/verify-email` - verify email

**User routes**:\
`POST /1/users` - create a user\
`GET /1/users` - get all users\
`GET /1/users/:userId` - get user\
`PATCH /1/users/:userId` - update user\
`DELETE /1/users/:userId` - delete user
...
An access token can be generated by making a successful call to the register (`POST /1/auth/register`) or login (`POST /1/auth/login`) endpoints. The response of these endpoints also contains refresh tokens (explained below).

An access token is valid for 30 minutes. You can modify this expiration time by changing the `JWT_ACCESS_EXPIRATION_MINUTES` environment variable in the .env file.

**Refreshing Access Tokens**:

After the access token expires, a new access token can be generated, by making a call to the refresh token endpoint (`POST /1/auth/refresh-tokens`) and sending along a valid refresh token in the request body. This call returns a new access token and a new refresh token.
