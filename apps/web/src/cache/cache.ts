...
/**
 * Schema definition for Team entities
 */
export const Team = new schema.Entity<TeamType>(
	'teams',
	{},
	{
		mergeStrategy: defaultMergeStrategy,
...
// Define relationships after both schemas exist
Team.define({
	memberships: [User],
});
 ...
 */
export type BoilerplateCache = {
	users: {
		[k: string]: EntityType<typeof User> | undefined;
	};
	teams: {
		[k: string]: EntityType<typeof Team> | undefined;
	};
};
...
const _cache = create<
	BoilerplateCache & {
		_update: (nextState: BoilerplateCache) => void;
		reset: () => void;
	}
>((setState, getState) => ({
	users: {},
	...
	reset: () => {
		...
		Object.keys(nextCache).forEach((key) => {
			nextCache[key as keyof BoilerplateCache] = {};
		});
...
const _httpCache = create<{
	...
	reset: () => setState({ requests: {}, lastTokenRefresh: null }),
}));
 ...
 * @param {string} entityName - Type of entity being set
 * @param {object} entity - The entity data
 * @returns {object} Updated cache state
 */
const maintainBidirectionalRelationships = (
	cache: BoilerplateCache,
	entityName: SingularModelName,
	entity: any,
): BoilerplateCache => {
	const relationships = RELATIONSHIP_MAP[entityName as keyof typeof RELATIONSHIP_MAP];
	if (!relationships) {
		return cache;
	}
	...
	Object.entries(relationships).forEach(([relationshipName, config]) => {
		...
		const inverseField = config.inverseField
			? config.inverseField
			: SingularToPluralModelName[entityName];
		...
		if (foreignKeyValue) {
			...
			if (createReference && !entity[createReference]) {
				const entityPluralType = SingularToPluralModelName[entityName] as keyof BoilerplateCache;
...
class Cache {
	...
	set<Model extends SingularModelName>(
		model: Model,
		data:
			| BoilerplateCache[(typeof SingularToPluralModelName)[Model]][string]
			| BoilerplateCache[(typeof SingularToPluralModelName)[Model]][string][]
			| null,
	) {
		if (Array.isArray(data)) {
		...
		const normalized = normalize<
			EntityType<BoilerplateCache[(typeof SingularToPluralModelName)[Model]][string]>
		...
		const modelNames = Object.keys(nextEntities) as (keyof BoilerplateCache)[];
	 ...
	 */
	async create<Model extends (typeof PluralToSingularModelName)[keyof BoilerplateCache]>(
		model: Model,
		{
			url,
			method = 'POST',
			...
			merge?: (
				data: any,
				current: BoilerplateCache[SingularToPluralModelNameMap[Model]][string],
			) => any;
			signal?: Request['signal'];
			didRetry?: boolean;
		},
	): Promise<
		[BoilerplateCache[SingularToPluralModelNameMap[Model]][string] | null, Error | null, Response]
	> {
		await this.refreshTokens();
		...
		const json = (await response.json()) as
			| BoilerplateCache[keyof BoilerplateCache][string]
			| ApiError;
	 ...
	 * @returns {Promise<[object|null, Error|null, Response]>} Tuple of [entity, error, response]
	 */
	async update<Model extends (typeof PluralToSingularModelName)[keyof BoilerplateCache]>(
		model: Model,
		{
			url,
			method = 'PUT',
	...
	/**
	 * Delete an existing entity via API and store it in the cache
	 * Example:
	 * const [user, error, response] = await cache.delete('user', {
	 *   url: '/1/users',
	 *   data: { name: 'John Doe' },
	 * });
	 * @param {ModelName} model - The type of model to update
	 ...
	 * @returns {Promise<[object|null, Error|null, Response]>} Tuple of [entity, error, response]
	 */
	async delete<Model extends (typeof PluralToSingularModelName)[keyof BoilerplateCache]>(
		model: Model,
		{
			url,
			method = 'DELETE',
	...
	clear() {
		this.cache.getState().reset();
		this.httpCache.getState().reset();
	}
}
...
export const useModel = <
	Model extends (typeof PluralToSingularModelName)[keyof BoilerplateCache],
	SelectorResult,
	Entity extends BoilerplateCache[SingularToPluralModelNameMap[Model]][string],
>(
	modelName: Model,
	...
	return cache.cache(
		useShallow<BoilerplateCache, SelectorResult>((state) => {
			if (!id) {
				return selector(null);
			}

			const entityKey = SingularToPluralModelName[modelName] as keyof BoilerplateCache;
			// @ts-expect-error - TODO: fix this
			const schema = SingularModelNameToSchema[modelName] as BoilerplateCache[Model][string];
...
export const useModels = <
	Model extends keyof BoilerplateCache,
	SelectorResult,
	Entity extends NonNullable<BoilerplateCache[Model][string]>,
>(
	modelName: Model,
	...
	return cache.cache(
		useShallow<BoilerplateCache, SelectorResult>((state) => {
			// @ts-expect-error - TODO: fix this
			const schema = PluralModelNameToSchema[modelName] as BoilerplateCache[Model][string];
			// @ts-expect-error - TODO: fix this
