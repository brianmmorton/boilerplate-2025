...
describe.skip('Auth routes', () => {
	...
	describe('POST /1/auth/register', () => {
		let newUser: { email: string; firstName: string; lastName: string; password: string };
		beforeEach(() => {
			newUser = {
		...
		test('should return 400 error if email is invalid', async () => {
			...
			await request(app).post('/1/auth/register').send(newUser).expect(httpStatus.BAD_REQUEST);
		});
	...
	describe('POST /1/auth/logout', () => {
		...
		test('should return 400 error if refresh token is missing from request body', async () => {
			await request(app).post('/1/auth/logout').send().expect(httpStatus.BAD_REQUEST);
		});
		...
		test('should return 404 error if refresh token is not found in the database', async () => {
			...
			await request(app).post('/1/auth/logout').send({ refreshToken }).expect(httpStatus.NOT_FOUND);
		});
	...
	describe('POST /1/auth/refresh-tokens', () => {
		...
		test('should return 400 error if refresh token is missing from request body', async () => {
			await request(app).post('/1/auth/refresh-tokens').send().expect(httpStatus.BAD_REQUEST);
		});
	...
	describe('POST /1/auth/forgot-password', () => {
		beforeEach(() => {
		...
		test('should return 400 if email is missing', async () => {
			...
			await request(app).post('/1/auth/forgot-password').send().expect(httpStatus.BAD_REQUEST);
		});
	...
	describe('POST /1/auth/reset-password', () => {
	...
	describe('POST /1/auth/send-verification-email', () => {
		beforeEach(() => {
			jest.spyOn(emailService.transport, 'sendMail').mockClear();
		});
		...
		test('should return 204 and send verification email to the user', async () => {
			...
			await request(app)
				.post('/1/auth/send-verification-email')
	...
	describe('POST /1/auth/verify-email', () => {
		...
		test('should return 400 if verify email token is missing', async () => {
			...
			await request(app).post('/1/auth/verify-email').send().expect(httpStatus.BAD_REQUEST);
		});
